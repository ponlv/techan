...
func TestTimeSeries_AddCandle(t *testing.T) {
	...
	t.Run("Adds candle if last is nil", func(t *testing.T) {
		...
		candle := NewCandle(
			NewTimePeriod(time.Now(), time.Minute),
			WithClosePrice(big.NewDecimal(1)),
		)
	...
	t.Run("Does not add candle if before last candle", func(t *testing.T) {
		...
		now := time.Now()
		candle := NewCandle(NewTimePeriod(now, time.Minute), WithClosePrice(big.NewDecimal(1)))

		ts.AddCandle(candle)
		then := now.Add(-time.Minute * 10)

		nextCandle := NewCandle(NewTimePeriod(then, time.Minute), WithClosePrice(big.NewDecimal(2)))
...
func TestTimeSeries_LastCandle(t *testing.T) {
	...
	newCandle := NewCandle(NewTimePeriod(next, time.Minute), WithClosePrice(big.NewDecimal(2)))
